...
def update_all_stock_data_simple(start_date='20240410', end_date='20240412', all_stock_file_date='20240415'):
    ...
    for code in stock_latest_data['代码']:
        ...
        file_name = f"D:\\abu\\hk\\stock\\{code}_{start_date}_{all_stock_file_date}"
        stock_data.to_csv(file_name, index=False)

        # # 删除文件，不删除文件可以进行每天测试
        file_name = f"D:\\abu\\hk\\stock\\{code}_{start_date}_{end_date}"
        if os.path.exists(file_name):
            os.remove(file_name)
...
def check_choose_stock_change(get_data_date):
    ...
    if stock_latest_data is None:
        print("当前数据是空，请确认")
        return
    check_path = f"D:\\abu\\hk\\result\\choose_hk_stock_ang.csv"
    ...
    check_pd.to_csv(f"D:\\abu\\hk\\result\\choose_hk_stock_ang.csv"
