...
# 从csv文件获取指数价格数据
index_data = pd.read_csv('date/四种指数价格历史数据.csv').set_index('datetime')
...
df['asset_n'] = df['asset'].shift(N)
df['N_day_ret'] = df['asset'] / df['asset_n'] - 1
...
df.loc[:, ['asset', 'stgy']].plot(ax=ax1, grid=True,
...
# 从csv文件获取期货价格数据
future_data = pd.read_csv('date/期货主连数据.csv').set_index('datetime')
...
df['stgy_ret'] = df['ret'] * df['pos']
df['stgy'] = (1 + df['stgy_ret']).cumprod().fillna(1)
df.loc[:, ['asset', 'pos', 'stgy']].plot(figsize=(16, 8), subplots=True, grid=True,
                                         title='Boll Band on %s, N=%d' % (future_name, N))
res = cal_period_perf_indicator(df.loc[:, ['asset', 'stgy']])
print(res)
