...
def get_all_latest_stock():
    ...
    file_name = f"D:\\abu\\cn\\all\\{current_date}.csv"
    if os.path.exists(file_name):
        ...
        stock_data['代码'] = stock_data['代码'].astype(str)
...
def pick_stock(stock_code_data, start_date='20230101', end_date='20240410'):
    ...
    pick_result_df.to_csv(f"D:\\abu\\cn\\result\\choose_a_stock.csv", mode='a', header=False, encoding='utf-8', index=False)
    # pick_result_df.to_csv(f"D:\\abu\\cn\\result\\choose_a_stock_{end_date}.csv", mode='w', header=True, encoding='utf-8',index=False)
...
def check_choose_stock_change(get_data_date):
    ...
    if stock_latest_data is None:
        print("当前数据是空，请确认")
        return
    check_path = f"D:\\abu\\cn\\result\\choose_a_stock.csv"
    ...
    for j in check_pd['code']:
        ...
        latest_data = stock_latest_data.loc[stock_latest_data['代码'] == int(j)]
    ...
    check_pd.to_csv(f"D:\\abu\\cn\\result\\choose_a_stock.csv"
